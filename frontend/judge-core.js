...
(function () {
  ...
  /* ------------------------------------------------------------
     DIFFICULTY  ✅ PATCHED: Undo behavior same as RE
     - Undo starts hidden
     - After any tap → Undo shows
     - After undo → Undo hides + lastAction cleared
  ------------------------------------------------------------ */
    ...
    init() {
      ...
      document.querySelectorAll(".skill-btn").forEach(btn => {
        btn.addEventListener("pointerdown", () => {
          ...
          // snapshot like RE: store prev before change
          this.lastAction = { level: lvl, prev: this.counts[lvl] };
          this.counts[lvl]++;

          this.updateUI();

          // ✅ match RE: show Undo only after an action
      ...
      const undoHandler = () => {
        ...
        // ✅ match RE: hide Undo after undoing
        if (this.undoBtn) this.undoBtn.classList.add("hidden");
      ...
      if (this.undoBtn) {
        this.undoBtn.addEventListener("pointerdown", undoHandler, { passive: true });
      }

      // RESET
      if (this.resetBtn) {
        this.resetBtn.addEventListener("pointerdown", () => {
          for (let lvl in this.counts) this.counts[lvl] = 0;
          this.lastAction = null;
          this.updateUI();

          // ✅ match RE: hide Undo after reset
          if (this.undoBtn) this.undoBtn.classList.add("hidden");
